...
public class CosineSimilarity {
	...
	public double Cosine_Similarity_Score(String Text1, String Text2) {
		...
		for (int i = 0; i < word_seq_text1.length; i++) {
			String tmp_wd = word_seq_text1[i].trim().replace("-", " ");
			if (tmp_wd.length() > 0) {
		...
		for (int i = 0; i < word_seq_text2.length; i++) {
			String tmp_wd = word_seq_text2[i].trim().replace("-", " ");
			if (tmp_wd.length() > 0) {
				if (word_freq_vector.containsKey(tmp_wd)) {
					values vals1 = word_freq_vector.get(tmp_wd);
		...
		/*Set<String> keys = word_freq_vector.keySet();
		for(String key: keys){
		...
		if(Double.isNaN(sim_score)) return 0;
		else return sim_score
				;
	}
	...
	public static void main(String[] args) {
		...
		String _1 = "Robert Tibshirani";
		_1 = _1.replace(",", "").replace("-", " ");
		_1 = _1.toLowerCase();
		StopWords sw = new StopWords();
		sw.get_clean_text(_1);
